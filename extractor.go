...
func (e *Extractor) extractPdf(filePath string) (string, error) {
	...
	// 简单PDF文本提取：查找BT...ET之间的文本
	content := extractPDFText(string(data))
	if len([]rune(content)) > maxContentLen {
		content = string([]rune(content)[:maxContentLen])
	}
...
func extractPDFText(content string) string {
	...
	for _, line := range lines {
		...
		if inText {
			// 提取Tj和TJ操作符中的文本
			if strings.Contains(trimmed, "Tj") {
				// 提取括号内的文本
				start := strings.Index(trimmed, "(")
				end := strings.LastIndex(trimmed, ")")
				if start >= 0 && end > start {
					buf.WriteString(trimmed[start+1 : end])
				}
			}
			if strings.Contains(trimmed, "TJ") {
				// 复杂的TJ数组，简单处理
				buf.WriteString(extractTJText(trimmed))
			}
		}
	}
	return buf.String()
...
func extractTJText(s string) string {
	var buf bytes.Buffer
	inParen := false
	for _, r := range s {
		if r == '(' {
